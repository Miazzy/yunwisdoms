//免登陆白名单配置 // no redirect whitelist
const whiteList = [
    '/user/login',
    '/user/register',
...
window.viewSubNode = viewSubNode;
...
window.accountNode = accountNode;
...
window.isystemNode = isystemNode;


const handleRouter = (to, from, next, NProgress, types, store, utils, router) => {
    ...
    try {
        if (Vue.ls.get(types.ACCESS_TOKEN)) {
            /* has token */
                ...
                if (store.getters.permissionList.length === 0) {
                    store
                        ...
                        .then(res => {
                            ...
                            //如果首页模块，则添加此模块
                            if (!homeFlag && !window.homeNode) {
                                res.result.menu.push(window.homeNode);
                            }
                            //如果没有个人管理模块，则添加此模块
                            if (!accountFlag && !window.accountNode) {
                                res.result.menu.push(window.accountNode);
                            }
                            //如果没有审批管理模块，则添加此模块
                            if (!workFlowFlag && !window.workFlowNode) {
                                res.result.menu.push(window.workFlowNode);
                            }
                            //如果没有行政管理模块，则添加此模块
                            if (!adminFlag && !window.adminNode) {
                                res.result.menu.push(window.adminNode);
                            }
                            //如果没有系统管理模块，则添加此模块
                            if (!isystemFlag && !window.isystemNode) {
                                res.result.menu.push(window.isystemNode);
                            }
                            //如果没有在线表单开发模块，则添加此模块
                            if (!onlineFlag && !window.onlineNode) {
                                res.result.menu.push(window.onlineNode);
                            }
                            ...
                            let constRoutes = [];
                            constRoutes = utils.generateIndexRouter(menuData);
                            // 添加主界面路由
...
window.handleRouter = handleRouter;